...
class SuperBizAgentApp {
    ...
    bindEvents() {
        ...
        // AI Ops按钮 - 显示诊断类型选择对话框
        if (this.aiOpsSidebarBtn) {
            this.aiOpsSidebarBtn.addEventListener('click', () => this.showDiagnosisModal());
        }
        
        // 诊断模态框关闭按钮
        if (this.modalCloseBtn) {
            this.modalCloseBtn.addEventListener('click', () => this.closeDiagnosisModal());
        }
        
        // 诊断选项点击
        if (this.diagnosisOptions) {
            this.diagnosisOptions.forEach(option => {
                option.addEventListener('click', () => {
                    const type = option.getAttribute('data-type');
                    this.selectDiagnosisType(type);
                });
            });
        }
        
        // 点击模态框背景关闭
        if (this.diagnosisModal) {
    ...
    // 发送智能运维请求（SSE 流式模式）
    async sendAIOpsRequest(loadingMessageElement, diagnosisType = 'system', questionContext = null) {
        try {
            const requestBody = {
            ...
            // 如果是问题诊断，添加上下文
            if (diagnosisType === 'question' && questionContext) {
                requestBody.question_context = questionContext;
            }
    ...
    async triggerAIOps() {
        ...
        if (this.currentChatHistory.length > 0) {
            // 获取最近的对话内容作为上下文
            const recentMessages = this.currentChatHistory.slice(-4); // 最近4条消息
            questionContext = recentMessages.map(msg => 
                `${msg.type === 'user' ? '用户' : '助手'}: ${msg.content}`
            ).join('\n\n');
            diagnosisType = 'question';
                
            this.showNotification('检测到对话历史，将分析问题...', 'info');
        } else {
            this.showNotification('将进行系统诊断...', 'info');
        }
        ...
        // 添加“分析中...”的消息（带旋转动画）
        const loadingMessage = this.addLoadingMessage(diagnosisType === 'question' ? '问题分析中...' : '系统诊断中...');
    ...
    selectDiagnosisType(type) {
        ...
        this.showNotification(`已选择${typeNames[type]}模式`, 'info');
        
        // 根据诊断类型执行不同的逻辑
        if (type === 'system') {
            // 系统诊断：检查是否有对话历史
            if (this.currentChatHistory.length > 0) {
                this.showNotification('系统将分析当前会话上下文进行诊断', 'info');
            }
            this.triggerAIOps();
        } else if (type === 'question') {
            // 问题诊断：需要有对话内容
            if (this.currentChatHistory.length === 0) {
                this.showNotification('请先在对话中描述您的问题，然后再使用问题诊断', 'warning');
                return;
            }
            this.triggerQuestionDiagnosis();
        }
    }
    
    // 触发问题诊断
    async triggerQuestionDiagnosis() {
        if (this.isStreaming) {
            this.showNotification('请等待当前操作完成', 'warning');
            return;
        }
        ...
        // 添加“问题分析中...”的消息
        const loadingMessage = this.addLoadingMessage('问题分析中...');
        ...
        try {
            // 构造问题诊断的提示词
            const diagnosisPrompt = `请对以下对话中的问题进行深度诊断分析：
...
4. 如果需要进一步排查，给出具体步骤`;
            ...
            // 使用流式对话接口进行分析
            await this.sendStreamMessage(diagnosisPrompt);
        } catch (error) {
            console.error('问题诊断失败:', error);
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，问题诊断时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
